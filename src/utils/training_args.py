...
class MoreOptimizerNames(ExplicitEnum):
    """
    ...
    ADAMW_SCALE = "adamwscale"



@dataclass
class CustomTrainingArguments(Seq2SeqTrainingArguments): 
    final_cosine: float = field(
        default=1e-5, 
        metadata={"help": "Minimum learning rate. https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.CosineAnnealingLR.html"}
    )
    def __post_init__(self):
        ...
        self.optim = MoreOptimizerNames(self.optim)
        if self.adafactor:
            warnings.warn(
            ...
            self.optim = MoreOptimizerNames.ADAFACTOR
        if self.optim == MoreOptimizerNames.ADAMW_TORCH_FUSED and is_torch_available():
